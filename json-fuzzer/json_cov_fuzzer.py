...
with Coverage() as cov_fuzz:
    try:
        json_parser_mutated_1.value_parser(baseline_fuzz.fuzz().strip())
    except:
        pass
cov_prob_grammar = mutant_grammar_gen.random_vector_gen(cov_prob_grammar, "sample", samples)
random_prob_grammar = mutant_grammar_gen.random_vector_gen(cov_prob_grammar, "sample", samples)
...
for j in range(int(sys.argv[1])): #limit iterations of fuzzer
    ...
    for fi in inputs:
        ...
        json_inp_file = open("json_inp_1.json","w")
        ...
        for i,m in enumerate(mutant_srcs):   
            ...
            mutated_prog = m
            mutated_file = open('json_parser_mutated_1.py','w')
            mutated_file.write(mutated_prog)
            mutated_file.close()
        
            with Coverage() as cov_fuzz:
                try:
                    json_parser_mutated_1.value_parser(fi.strip())
                except:
                    pass
            correct_cov = len(cov_fuzz.coverage())
            out = subprocess.Popen([sys.executable,"json_parser_test_1.py"],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
