...
logging.basicConfig(filename='jinja-fuzzer-mut-'+str(sys.argv[1])+'-'+str(sys.argv[2])+'-'+str(sys.argv[3])+'.log', 
...
def install_mutant(runner, mutant, mutant_path):
    ...
    f.write(mutant)
    f.close()
    runner.exec_run("cp -r ./jinja_mutate /")
...
def test_env(runner,samples,ind):
    ...
    for sample in samples:
        samplef=open("template_file_"+str(ind),"w")
        samplef.write(sample)
        samplef.close()
        rand_int = random.randint(0,50)
        r = runner.exec_run("python3 jinja_tester.py "+str(ind)+" "+ str(rand_int))
        mut_out = r.output.decode("utf-8")
        out = subprocess.Popen([sys.executable, "jinja_tester.py",str(ind),str(rand_int)],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
template_path = "jinja_mutate/jinja2/environment.py"
template_to_fuzz = open(template_path)

template_seed = template_to_fuzz.read()
...
mut_prob_grammar = copy.deepcopy(JINJA_GRAMMAR)
random_prob_grammar = copy.deepcopy(JINJA_GRAMMAR)
...
if strategy == "mixed":
    ...
    for mutant in mutant_creator.MuFunctionAnalyzer(environment_mutate, strategy="return"): ## fn[0] is the function name. We need to pass the function, so take fn[1]
        ret_mut.append(mutant.src())

    for mutant in mutant_creator.MuFunctionAnalyzer(environment_mutate, strategy="condition"): ## fn[0] is the function name. We need to pass the function, so take fn[1]
        if_mut.append(mutant.src())

    for mutant in mutant_creator.MuFunctionAnalyzer(environment_mutate, strategy="ds"): ## fn[0] is the function name. We need to pass the function, so take fn[1]
    ...
    for mutant in mutant_creator.MuFunctionAnalyzer(environment_mutate, strategy=strategy): ## fn[0] is the function name. We need to pass the function, so take fn[1]
            ...
            while runner_ind < scale_factor:
                if ind not in killed_list:
                    if ind < len(mutant_srcs):
                        #install mutant
                        install_mutant(runners[runner_ind], mutant_srcs[ind], template_path)
                        #start thread
...
plt.title("Jinja Mut Guided Fuzzer")
...
plt.grid(True)
plt.savefig(strategy+"/jinja_mut/mutant_"+str(sys.argv[1])+"_"+str(sys.argv[2])+"_"+str(sys.argv[3])+".png")
plt.clf()


plt.plot(x_cov,y_cov,color='green', marker='o')
plt.title("Jinja Cov Guided Fuzzer")
plt.xlabel("Number of total iterations")
plt.ylabel("Coverage")
plt.xticks(range(len(x_cov)+1))
plt.grid(True)
plt.savefig(strategy+"/jinja_mut/cov_"+str(sys.argv[1])+"_"+str(sys.argv[2])+"_"+str(sys.argv[3])+".png")
plt.clf()
