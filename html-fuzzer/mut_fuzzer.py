...
def test_env(runner,samples,ind):
    ...
    for sample in samples:
        ...
        logging.info("for original code input:"+sample+":original code output:"+str(org_out))
        logging.info("For mutated code input:"+sample+":mutated code output:"+str(mut_out))
        if len(org_out) == 2:
            if len(mut_out) == 2:
...
mut_prob_grammar = copy.deepcopy(HTML_GRAMMAR)
...
plt.title("HTML Mut Guided Fuzzer")
...
plt.grid(True)
plt.savefig(strategy+"/html_mut/mutant_"+str(sys.argv[1])+"_"+str(sys.argv[2])+"_"+str(sys.argv[3])+".png")
plt.clf()
...
plt.grid(True)
plt.savefig(strategy+"/html_mut/cov_"+str(sys.argv[1])+"_"+str(sys.argv[2])+"_"+str(sys.argv[3])+".png")
plt.clf()
